...
export default function AdminProcessesPage() {
  ...
  async function changeStage(id: string, stage: EtapaProcesso, motivo?: string | null) { try { await updateApplicationStage(id, stage, motivo); setApplications((current) => current.map((item) => item.id === id ? { ...item, etapa: stage, observacoes: motivo === undefined ? item.observacoes : motivo, updated_at: new Date().toISOString() } : item)); setMessage("Etapa atualizada com sucesso."); } catch (error) { if (import.meta.env.DEV) console.error(error); setMessage("Não foi possível atualizar a etapa."); } }
