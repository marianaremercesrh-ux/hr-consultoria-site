...
export default function AdminProcessesPage() {
  ...
  async function changeStage(id: string, stage: EtapaProcesso, motivo?: string | null) { try { const saved=await updateApplicationStage(id, stage, motivo); setApplications((current) => current.map((item) => item.id === id ? { ...item, etapa: saved.etapa, observacoes: saved.observacoes, updated_at: saved.updated_at } : item)); setMessage("Etapa atualizada com sucesso."); return saved; } catch (error) { if (import.meta.env.DEV) console.error(error); setMessage("Não foi possível atualizar a etapa."); throw error; } }
