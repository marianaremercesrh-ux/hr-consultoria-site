...
export default function AdminCandidateProfilePage({ id }: { id: string }) {
  ...
  return <main className="min-h-screen bg-[#F5F7FA]"><AdminNav/><section className="mx-auto max-w-6xl px-5 py-10">
    ...
    {applications.map(application=><section key={application.id} className="mt-8"><PortalReleaseControl application={application} onChanged={load}/><CandidateProcessTracking application={application} candidateId={id} onSaved={load}/></section>)}
