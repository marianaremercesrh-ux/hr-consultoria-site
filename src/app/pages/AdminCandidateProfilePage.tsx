...
import { ETAPAS, type Candidato, type CandidaturaDetalhada, type EtapaProcesso } from "../types/candidates";
...
export default function AdminCandidateProfilePage({ id }: { id: string }) {
  ...
  async function changeStage(applicationId: string, value: EtapaProcesso) { try { await updateApplicationStage(applicationId, value); setMessage("Etapa atualizada com sucesso."); await load(); } catch (error) { if (import.meta.env.DEV) console.error(error); setMessage("Não foi possível atualizar a etapa."); } }
  async function addApplication() { if (!jobId) return; try { await createApplication(id, jobId, stage); setJobId(""); setMessage("Candidato adicionado à vaga."); await load(); } catch (error) { if (import.meta.env.DEV) console.error(error); setMessage("Não foi possível adicionar o candidato à vaga."); } }
