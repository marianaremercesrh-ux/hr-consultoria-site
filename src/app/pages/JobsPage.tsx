...
function toPublicJob(job: PublicJobRecord): PublicJob {
  return {
    ...
    contract: job.tipo_contrato || "NÃ£o informado",
    modality: job.modalidade || "NÃ£o informada",
    ...
    schedule: job.horario || "NÃ£o informado",
...
function JobsHeader() {
  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-border shadow-sm">
    <div className="mx-auto flex min-h-[120px] max-w-7xl items-center justify-between gap-6 px-5 py-5 sm:px-6 lg:px-10">
      <a href="/#inicio" className="flex items-center" aria-label="Ir para o início"><Logo variant="color" showText={false} className="h-[64px] w-auto max-w-none shrink-0 sm:h-[65px]" /></a>
      <nav className="hidden xl:flex flex-1 items-center justify-center gap-10" aria-label="Menu principal">{NAV_LINKS.map((link) => <a key={link.href} href={link.href} aria-current={link.href === "/vagas" ? "page" : undefined} className={`relative text-[17px] font-semibold tracking-wide transition-colors after:absolute after:-bottom-2 after:left-0 after:h-0.5 after:bg-[#D4A62A] after:transition-all ${link.href === "/vagas" ? "text-[#D4A62A] after:w-full" : "text-[#052656] hover:text-[#D4A62A] after:w-0 hover:after:w-full"}`}>{link.label}</a>)}</nav>
    ...
    <footer style={{ backgroundColor: "#052656" }} className="py-16">
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-14">
          <div>
            ...
            <p className="text-lg leading-relaxed max-w-xs" style={{ color: "rgba(255,255,255,0.52)" }}>
              A HR Gestão e Soluções oferece recrutamento e seleÃ§Ã£o com processo claro, triagem cuidadosa e comunicaÃ§Ã£o prÃ³xima.
            </p>
          </div>

          <div>
            <p className="text-base font-medium tracking-[0.15em] uppercase text-white/45 mb-5">NavegaÃ§Ã£o</p>
            <ul className="space-y-3" lang="pt-BR" translate="no">
              ...
              <li>
                <a href="/admin/login" className="group inline-flex items-start gap-2 text-white/55 transition-colors hover:text-[#D4A62A]">
                  ...
                  <span><strong className="block text-lg">Ãrea do Recrutador</strong><span className="block text-sm text-white/40 group-hover:text-white/60">GestÃ£o administrativa e processos seletivos</span></span>
                </a>
              </li>
              <li>
          ...
          <div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" aria-label="Abrir Instagram oficial da HR Gestão e Soluções em nova aba" className={`${INSTAGRAM_BUTTON_CLASS} w-fit`}>
        ...
        <div className="pt-8 border-t text-center text-base leading-relaxed" style={{ borderColor: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.3)" }}>
          Â© HR Gestão e Soluções. Todos os direitos reservados.
        </div>
      </div>
    </footer>
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (active) setError("NÃ£o foi possÃ­vel carregar as vagas.");
      } finally {
