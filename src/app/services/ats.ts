...
export async function updateInterviewStatus(id:string,status:Entrevista["status"]){const{data,error}=await supabase.from("entrevistas").update({status,updated_at:new Date().toISOString()}).eq("id",id).select(ENTREVISTA_COLUMNS).single();if(error)throw error;return data as Entrevista;}
