...
function cleanDisplayText(value:string){return value.replace(/movÃ©is/giu,"mÃ³veis").replace(/vou encaminhar o currÃ­culo dele para vocÃª avaliar[.!]?/giu,"").replace(/\s{2,}/g," ").trim()}
function shortText(value:string){const clean=cleanDisplayText(value);return clean.length>240?`${clean.slice(0,237).trimEnd()}â€¦`:clean}
async function assetDataUrl(path:string){const response=await fetch(path);if(!response.ok)throw new Error(`NÃ£o foi possÃ­vel carregar o recurso ${path}.`);const blob=await response.blob();return await new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result));reader.onerror=()=>reject(reader.error);reader.readAsDataURL(blob)})}

async function svgDataUrlToPng(svgDataUrl:string){return await new Promise<string>((resolve,reject)=>{const image=new Image();image.onload=()=>{const canvas=document.createElement("canvas");canvas.width=2340;canvas.height=1460;const context=canvas.getContext("2d");if(!context){reject(new Error("NÃ£o foi possÃ­vel preparar a logo do relatÃ³rio."));return}context.drawImage(image,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL("image/png"))};image.onerror=()=>reject(new Error("NÃ£o foi possÃ­vel renderizar a logo do relatÃ³rio."));image.src=svgDataUrl})}
...
export async function downloadCandidatesPdf(rows:CandidateReportRow[],filters:string[],kind:CandidateReportKind,includeContact:boolean){
  ...
  const filterText=filters.length?filters.join(" â€¢ "):"Nenhum filtro aplicado";
  let y=0;
  function header(first:boolean){doc.setFillColor(5,38,86);doc.rect(0,0,width,28,"F");doc.addImage(logo,"PNG",left,7.4,21.17,13.2,undefined,"FAST");doc.setTextColor(255);doc.setFontSize(13);doc.text("RelatÃ³rio de candidatos",42.3,15.5);y=35;if(first){doc.setTextColor(70);doc.setFontSize(8.5);doc.text(`Tipo: RelatÃ³rio ${kind}`,left,y);y+=5;doc.text(`Emitido em: ${emittedAt}`,left,y);y+=5;const filterLines=doc.splitTextToSize(`Filtros: ${filterText}`,contentWidth);doc.text(filterLines,left,y);y+=filterLines.length*4.2;doc.text(`Total de candidatos: ${rows.length}`,left,y);y+=8}}
  ...
  rows.forEach((row,index)=>{
    ...
    const analysisLines:string[]=doc.splitTextToSize(analysis||"Nenhuma anÃ¡lise registrada.",contentWidth-12);
    ...
    while(remaining.length){
      ...
      doc.setDrawColor(210);doc.setFillColor(index%2===0?248:255,249,250);doc.roundedRect(left,y,contentWidth,blockHeight-3,2,2,"FD");doc.setFillColor(212,166,42);doc.rect(left,y,2.2,blockHeight-3,"F");doc.setTextColor(5,38,86);doc.setFontSize(first?11.5:9.5);doc.text(first?row.nome:`${row.nome} â€” continuaÃ§Ã£o`,left+6,y+7);
      let textY=y+12;if(first){doc.setTextColor(55);doc.setFontSize(8.5);doc.text(`Vaga: ${row.vaga}`,left+6,textY,{maxWidth:112});doc.text(`Status: ${row.status}`,left+122,textY,{maxWidth:58});textY+=5;doc.text(`Cadastro: ${row.cadastro}`,left+6,textY);textY+=6;doc.setTextColor(5,38,86);doc.text(kind==="resumido"?"Resumo":"Resumo da entrevista/anÃ¡lise",left+6,textY);textY+=4.5}else{textY+=1}
  ...
  const pages=doc.getNumberOfPages();for(let page=1;page<=pages;page++){doc.setPage(page);doc.setDrawColor(212,166,42);doc.line(left,height-12,width-right,height-12);doc.setTextColor(80);doc.setFont("NotoSans","normal");doc.setFontSize(8);doc.text(`PÃ¡gina ${page} de ${pages}`,width-right,height-7,{align:"right"})}
...
export async function downloadCandidatesExcel(rows:CandidateReportRow[],kind:CandidateReportKind,includeContact:boolean){
  const ExcelJS=await import("exceljs");const workbook=new ExcelJS.Workbook();workbook.creator="HR Solutions";workbook.created=new Date();const sheet=workbook.addWorksheet("Candidatos",{views:[{state:"frozen",ySplit:1}]});
  const columns:Array<{header:string;key:keyof CandidateReportRow;width:number}>=[{header:"Nome",key:"nome",width:28},{header:"Vaga",key:"vaga",width:38},{header:"Status atual",key:"status",width:28},{header:"Data de cadastro",key:"cadastro",width:18},{header:kind==="resumido"?"Resumo curto":"Resumo da entrevista/anÃ¡lise",key:"analise",width:65}];
