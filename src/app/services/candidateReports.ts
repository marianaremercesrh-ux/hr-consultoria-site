...
export async function downloadCandidatesPdf(rows:CandidateReportRow[],filters:string[],kind:CandidateReportKind,includeContact:boolean){
  ...
  let y=0;
  function header(first:boolean){doc.setFillColor(5,38,86);doc.rect(0,0,width,28,"F");doc.addImage(logo,"PNG",left,5,34,16,undefined,"FAST");doc.setTextColor(255);doc.setFontSize(13);doc.text("HR Consultoria de RH",52,15);y=35;if(first){doc.setTextColor(5,38,86);doc.setFontSize(19);doc.text("Relatório de candidatos",left,y);y+=8;doc.setTextColor(70);doc.setFontSize(8.5);doc.text(`Tipo: Relatório ${kind}`,left,y);y+=5;doc.text(`Emitido em: ${emittedAt}`,left,y);y+=5;const filterLines=doc.splitTextToSize(`Filtros: ${filterText}`,contentWidth);doc.text(filterLines,left,y);y+=filterLines.length*4.2;doc.text(`Total de candidatos: ${rows.length}`,left,y);y+=8}}
