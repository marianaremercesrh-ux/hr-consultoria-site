...
Configure os secrets `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` e `CLIENT_PORTAL_URL` somente no ambiente da função. Em produção, use `CLIENT_PORTAL_URL=https://hr-consultoria-site.vercel.app/cliente`. Nunca use `service_role` no Vite.
